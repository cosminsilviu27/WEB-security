...
if(isset($_GET['action']) && $_GET['action']=="add"){
	...
	if(isset($_SESSION['cart'][$id])){
		...
		$sql_p=$db->query("SELECT * FROM products WHERE id={$id}");
		//$query_p=mysqli_query($con,$sql_p);
		if(mysqli_num_rows($sql_p)!=0){
			$row_p=$sql_p->fetchAll(PDO::FETCH_ASSOC);
...
/*for($i=0; $i < 40; $i++) {
	$pw = md5($row[$i]);
	echo $pw;
	$q=$db->query("UPDATE `users` SET `password` = '$pw' WHERE `users`.`id` = '$i'");
}*/

$ret=$db->query("select * from products");
...
$ret=$db->query("select * from products where category=3");
$results=$ret->fetchAll(PDO::FETCH_ASSOC);
foreach ($results as $row) 
{
	# code...
...
$ret=$db->query("select * from products where category=5");
$results=$ret->fetchAll(PDO::FETCH_ASSOC);
foreach ($results as $row) 
{
?>
...
$ret=$db->query("select * from products where category=4 and subCategory=4");
$results=$ret->fetchAll(PDO::FETCH_ASSOC);
foreach ($results as $row) 
{
?>
...
$ret=$db->query("select * from products where category=4 and subCategory=6");
$results=$ret->fetchAll(PDO::FETCH_ASSOC);
foreach ($results as $row) 
{
?>
...
$ret=$db->query("select * from products where category=6");
$results=$ret->fetchAll(PDO::FETCH_ASSOC);
foreach ($results as $row)
{
	# code...
								...
								<div class="row product-micro-row">
									...
									<div class="col col-xs-6">
										<div class="product-info">
											...
											<div class="product-price">	
												<span class="price">
													Price. <?php echo htmlentities($row['productPrice']);?>
												</span>
